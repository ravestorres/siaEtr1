...
if (!isset($_SESSION['user_id'])) {
    echo "You need to be logged in to view proposals.";
    exit();
}
...
$stmt = $conn->prepare("SELECT * FROM proposals WHERE freelancer_id = ?");
        ...
        <?php
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // Fetch the job title separately
                    $job_id = $row['job_id'];
                    ...
                    // Check if the status is 'rejected' to disable the Edit Proposal button
                    $isRejected = ($row['status'] == 'rejected');

                    echo '<div class="card mb-4">';
                    echo '    <div class="card-header">';
                    echo '        <h5 class="card-title">' . htmlspecialchars($job_title) . '</h5>';
                    echo '    </div>';
                    echo '    <div class="card-body">';
                    echo '        <p class="card-text"><strong>Proposal Details: </strong>' . htmlspecialchars($row['proposal_details']) . '</p>';
                    echo '        <p class="card-text"><strong>Proposed Rate: </strong>$' . number_format($row['proposed_rate'], 2) . '</p>';
                    echo '        <p class="card-text"><strong>Status: </strong>' . (htmlspecialchars($row['status']) ?: 'Pending') . '</p>';
                    echo '        <p class="card-text"><strong>Created At: </strong>' . htmlspecialchars($row['created_at']) . '</p>';
                    
                    // Edit Proposal Button (disabled if rejected)
                    if ($isRejected) {
                        echo '        <button class="btn btn-warning btn-sm" disabled>Edit Proposal</button>';
                    } else {
                        echo '        <a href="edit_proposal.php?proposal_id=' . $row['id'] . '" class="btn btn-warning btn-sm">Edit Proposal</a>';
                    }

                    // Delete Proposal Button
                    echo '        <a href="delete_proposals.php?id=' . $row['id'] . '" class="btn btn-danger btn-sm ml-2" onclick="return confirm(\'Are you sure you want to delete this proposal?\')">Delete Proposal</a>';
